...
For GPU environments, the script auto-detects NVIDIA driver/CUDA capability and installs a compatible PyTorch wheel channel (tries newer CUDA wheels first, then older ones). On NVIDIA hosts, setup fails fast if no CUDA-compatible PyTorch can be initialized, so GPU tests are not silently downgraded to CPU mode.
...
- GPU benchmark skipped/failed:
  ...
  - If setup fails on an NVIDIA host, update GPU driver or temporarily run without GPU benchmark: `scripts/run_perfprobe.sh --run-only -- --no-gpu`
